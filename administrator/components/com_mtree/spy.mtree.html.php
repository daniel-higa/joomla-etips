<?php
/**
 * @version		$Id: spy.mtree.html.php 950 2010-11-16 09:07:27Z cy $
 * @package		Mosets Tree
...
class HTML_mtspy {
	...
	echo JText::sprintf( 'Analysis for actual rating', round(((($clone_votes-$removed_votes)/$listing->link_votes)*100),2), ($clone_votes-$removed_votes), $listing->link_votes, round((($listing->link_votes*$listing->link_rating)-(5*($clone_votes-$removed_votes)))/($listing->link_votes-($clone_votes-$removed_votes)),2)  );
	?></b>.</td></tr>
	<?php } ?>
	</table>
	<?php
				...
				<tr>
					...
					<td valign="top">
						...
						if(count($lists)>0) {
							...
							echo '<input type="button" onclick="removeClone()" value="' . JText::sprintf( 'Remove s including his her activities',$user->username ) . '" class="inputbox" />';
...
class mtfHTML {
	...
	function helpfuls( $vote_helpful, $vote_total ) {
		if ( $vote_total == 0 ) {
			...
			return JText::sprintf( 'Out of', $vote_helpful,  $vote_total );
		}
	}
