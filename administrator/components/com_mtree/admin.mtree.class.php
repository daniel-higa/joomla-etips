<?php
/**
 * @version		$Id: admin.mtree.class.php 1001 2011-01-18 03:00:20Z cy $
 * @package		Mosets Tree
		...
		if ( $link_approved < 0 ) {
			...
			if ( $mtconf->get('notifyuser_approved') == 1 ) {
				...
				if ( $email <> '' ) {
					...
					$body 		= JText::sprintf( 'Update listing approved msg', $this->link_name );
		...
		// Approval to new listing
			...
			if ( $mtconf->get('notifyuser_approved') == 1 ) {
				...
				if ( $email <> '' ) {
					...
					$body = JText::sprintf( 'New listing approved msg', $this->link_name );
					JUTility::sendMail( $MailFrom, $FromName, $email, $subject, $body );
		...
		if (!$this->_db->query()) {
			...
			if( $approve == 1 )
			{
				...
				if ( $mtconf->get('notifyuser_review_approved') == 1 ) {
					...
					$link_url = JURI::base() . 'index.php?option=com_mtree&task=viewlink&link_id=' . $row->link_id;

					if ( $row->email <> '' ) {
						$subject = JText::_( 'Review approved subject' );
						$body = JText::sprintf( 'Review approved msg', $row->link_name, $link_url );
				...
				if ( $mtconf->get('notifyowner_review_added') == 1 ) {
					...
					if ( $row->email <> '' ) {
						$subject = JText::sprintf( 'Review added subject', $row->link_name );
 						$body = JText::sprintf( 'Review added msg', $row->link_name, $link_url );
						JUTility::sendMail( $MailFrom, $FromName, $row->email, $subject, $body );
					}
				}
			}
...
class mtImage {
	...
	function check() {
		...
		if( !is_writable($this->directory) ) { 
			$this->setErrorMsg(JText::sprintf( 'Image dir not writable', $this->directory ));
			return false;
		}
