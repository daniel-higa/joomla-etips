<?php
/**
 * @version		$Id: spy.mtree.php 950 2010-11-16 09:07:27Z cy $
 * @package		Mosets Tree
...
function removeUserAndAllLogs( $option, $id ) {
	...
	$app->redirect('index.php?option=com_mtree&task=spy&task2=users', JText::_( 'User successfully removed' ) );
}
...
function removeLogs( $option, $cid ) {
	...
	if( count($cid) > 0 ) {
		...
		removeLogs2( $cid );
		$app->redirect('index.php?option=com_mtree&task=spy&task2=viewuser&id='.$user_id, JText::_( 'Activities successfully removed' ) );
	} else {
		viewUsers( $option );
	}
}
