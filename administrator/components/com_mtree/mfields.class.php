<?php
/**
 * @version		$Id: mfields.class.php 1030 2011-01-31 08:35:54Z cy $
 * @package		Mosets Tree
...
class mFieldType {
	...
	function getInputHTML() {
		if( !empty($this->arrayFieldElements[0]) )
		{
			...
			return '<input class="inputbox text_area'.($this->isRequired() ? ' required':'').'" type="text" name="' . $this->getInputFieldName(1) . '" id="' . $this->getInputFieldName(1) . '" size="' . ($this->getSize()?$this->getSize():'30') . '" value="' . htmlspecialchars($this->getValue()) . '" />';
		}
	}
	...
	function getOutput() {
		if( $this->tagSearch && $this->hasValue() )
		{
			...
			for($i=0;$i<$countTags;$i++)
			{
				...
				$outputTags[$i] = '';
				$outputTags[$i] .= '<a class="tag" href="'.JRoute::_('index.php?option=com_mtree&task=searchby&cf_id='.$this->getId().'&value='.urlencode($arrTags[$i]).'&Itemid='.$Itemid).'">';
...
class mFieldType_multitext extends mFieldType {
	function getInputHTML() {
		$html = '<textarea name="' . $this->getInputFieldName(1) . '" id="' . $this->getInputFieldName(1) . '" class="inputbox text_area" cols="60" rows="'.$this->getSize().'">' . htmlspecialchars($this->getValue()) . '</textarea>';
		return $html;
	}
	function getSearchHTML() {
	...
	function getInputHTML() {
		...
		foreach($this->arrayFieldElements AS $fieldElement) {
			if(!empty($fieldElement)) {
				...
				$html .= '<input type="radio" name="' . $this->getInputFieldName(1) . '" value="'.htmlspecialchars($fieldElement).'" id="' . $this->getInputFieldName(1) . '_' . $i . '" ';
				if( $fieldElement == $this->getValue() ) {
					$html .= 'checked ';
				}
	...
	function getSearchHTML() {
		...
		foreach($this->arrayFieldElements AS $fieldElement) {
			if(!empty($fieldElement)) {
				...
				$html .= '<input type="radio" name="' . $this->getName() . '" value="'.htmlspecialchars($fieldElement).'" id="' . $this->getName() . '_' . $i . '" ';
...
class mFieldType_checkbox extends mFieldType {
	...
	function getInputHTML() {
		...
		foreach($this->arrayFieldElements AS $fieldElement) {
			...
			$html .= '<input type="checkbox" name="' . $this->getInputFieldName(1) . '[]" value="'.htmlspecialchars($fieldElement).'" id="' . $this->getInputFieldName(1) . '_' . $i . '" ';
	...
	function getSearchHTML() {
		...
		$html = '';
		foreach($this->arrayFieldElements AS $fieldElement) {
			$html .= '<input type="checkbox" name="' . $this->getName() . '[]" value="'.htmlspecialchars($fieldElement).'" id="' . $this->getName() . '_' . $i . '" ';
			$html .= '/><label for="' . $this->getName() . '_' . $i . '">'.$fieldElement.'</label><br>';
			$i++;
		}
		return $html;
...
class mFieldType_file extends mFieldType {
	...
	*/	
	...
	function getInputHTML() {
		...
		$html .= '<input style="display:block;clear:both;float: none;" class="inputbox text_area" type="file" name="' . $this->getInputFieldName(1) . '" id="' . $this->getSearchFieldName(1) . '" />';
		return $html;
	}
...
class mFieldType_date extends mFieldType {
	...
	function getSearchHTML() {
		...
		$html = '';
		$html .= '<input style="float:none" id="' . $this->getSearchFieldName(1) . 'a" name=' . $this->getSearchFieldName(1) . ' type="radio" value="1" checked />';
