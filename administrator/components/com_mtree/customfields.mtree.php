<?php
/**
 * @version		$Id: customfields.mtree.php 950 2010-11-16 09:07:27Z cy $
 * @package		Mosets Tree
...
function uploadft( $option ) {
	...
	if(!empty($filename)) {
		...
		if( is_null($duplicate_ft_id) ) {
			...
			$app->redirect('index.php?option=com_mtree&task=managefieldtypes',JText::_( 'Field type installation success' ));
		} else {
			$app->redirect('index.php?option=com_mtree&task=managefieldtypes',JText::_( 'Field type upgraded successfully' ));
		}
...
function saveft( $ft_id, $option ) {
	...
	if ( $task == "applyft" ) {
		$app->redirect( "index.php?option=$option&task=editft&cfid=$ft_id" );
	} else {
		$app->redirect( "index.php?option=$option&task=managefieldtypes" );
	}
...
function removeft( $ft_id, $option ) {
	...
	if($fieldtype->iscore) {
		$app->redirect("index.php?option=$option&task=managefieldtypes",JText::_( 'Cannot delete core fieldtype' ));
	} else {
		if(!empty($field_type)) {
...
function removecf( $id, $option ) {
	...
	for ($i = 0; $i < count($id); $i++) {
		...
		if ($iscore == 1) {
			$app->redirect( "index.php?option=$option&task=customfields", JText::_( 'Cannot delete core field' ) );
		} else {
