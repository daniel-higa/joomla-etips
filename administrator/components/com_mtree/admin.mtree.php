<?php
/**
 * @version		$Id: admin.mtree.php 991 2011-01-13 01:20:38Z cy $
 * @package		Mosets Tree
...
if ( !$hide_menu && !in_array($task,array('upgrade','spy','ajax','downloadft', 'manageftattachments','editcat','editlink','newlink','editlink_for_approval')) ) {
...
switch ($task) {
	...
	*/
	case "globalupdate":
		...
		$app->redirect( "index.php?option=$option&task=listcats&cat_id=0", JText::_( 'Cat and listing count updated' ) );
		break;
...
function editlink( $link_id, $cat_id, $for_approval=false, $option ) {
	...
	$form->set('link_created', $row->link_created);
...
function savelink( $option, $post=null ) {
	...
	echo '$img_sort_hash: ' . $img_sort_hash;
...
function publishLinks( $link_id=null, $publish=1,  $option ) {
	...
	if (!is_array( $link_id ) || count( $link_id ) < 1) {
		...
		echo "<script> alert('".JText::sprintf( 'Select an item to', $action )."'); window.history.go(-1);</script>\n";
		exit;
	}
...
function removeLinks( $link_id, $option, $post=null ) {
	...
	if ( $returntask <> '' ) {
		$app->redirect( "index.php?option=$option&task=$returntask", sprintf(JText::_( 'Links have been deleted' ), count($link_id) ) );
	} else {
		$app->redirect( "index.php?option=$option&task=listcats&cat_id=".$row->cat_id );
	}
}
...
function moveCats2( $cat_id, $option ) {
	...
	if( in_array($new_cat_parent_id,$cat_id) ) {
		$app->redirect( "index.php?option=$option", JText::_('You can not move categories in to itself.') );
		return;
	}
...
function approve_links( $link_id, $publish=0, $option ) {
	...
	$app->redirect( "index.php?option=$option&task=listpending_links",sprintf(JText::_( 'Links have been aprroved' ),count( $link_id )) );	
}
...
function approve_cats( $cat_id, $publish=0, $option ) {
	...
	$app->redirect( "index.php?option=$option&task=listpending_cats",sprintf(JText::_( 'Cats have been aprroved' ),count( $cat_id )) );	
}
...
function save_pending_reviews( $option ) {
	...
	foreach( $reviews AS $review_id => $action ) {
		...
		if($action <> 0 && !empty($email_message[$review_id])) {
			$subject = JText::sprintf( 'Rejected approved review subject',$review_titles[$review_id] );
...
function save_claims( $option ) {
	...
	foreach( $claims AS $claim_id => $user_id ) {
		...
		if($user_id > 0) {
			...
			$body = JText::sprintf( 'Claim approved msg', $link->link_name, $mtconf->getjconf('live_site')."/index.php?option=com_mtree&task=viewlink&link_id=$link->link_id" );
	...
	if ( $search_where == 1 ) {
		...
		if ($id_found) {
			...
			if( !empty($link->link_name) ) {
				...
				$app->redirect( "index.php?option=com_mtree",JText::_( 'Your search does not return any result' ) );
			}
...
function template_pages( $option ) {
	...
	$form = new JParameter($template_params, $xmlfile);
	$form->loadINI($template_params);

	// HTML_mtree::template_pages( $template, $template_name, $params, $option );
	HTML_mtree::template_pages( $template, $template_name, $form, $option );
}
...
function edit_templatepage( $option ) {
	...
	if ( $fp = fopen( $file, 'r' ) ) {
		...
		$app->redirect( 'index.php?option='. $option .'&task=template_pages&template=' . $template, JText::sprintf( 'Cannot open file', $file ) );
	}
...
function copy_template2( $option ) {
	...
	if( $result === true )
	{
		...
		$app->redirect( 'index.php?option=com_mtree&task=templates', JText::_( 'Template successfully copied.' ) );
	}
...
function save_templatepage( $option ) {
	...
	if ( is_writable( $file ) == false ) {
		$app->redirect( "index.php?option=$option&task=templates" , JText::sprintf( 'File not writeable', $file ) );
	}
	...
	if ( $fp = fopen ($file, 'w' ) ) {
		...
		fclose( $fp );
		$app->redirect( $return, JText::_( 'Template page saved' ) );
	} else {
		$app->redirect( $return, JText::sprintf( 'File not writeable', $file ) );
	}
}
...
function install_template( $option ) {
	...
	if(!$zipfile->extract( PCLZIP_OPT_PATH, $tmp_install )) {
		$app->redirect( 'index.php?option=com_mtree&task=templates', JText::_( 'Template installation failed' ) );
	}
	...
	rmdirr($tmp_install);
	$app->redirect( 'index.php?option=com_mtree&task=templates', JText::_( 'Template installation success' ) );
...
function saveconfig($option) {
	...
	$app->redirect( "index.php?option=$option&task=config", JText::_( 'Config have been updated' ) );
}
