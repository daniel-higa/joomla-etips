<div class="reviews">
	...
	?>
	<div class="review"><div class="review-head"><?php
		
		?><div class="review-info"><?php 
		echo JText::_( 'Reviewed by' ) ?><span class="review-owner"><?php echo ( ($review->user_id) ? '<a href="' . JRoute::_('index.php?option=com_mtree&amp;task=viewusersreview&amp;user_id='.$review->user_id) . '">' . $review->username . '</a>': $review->guest_name); ?></span><p class="review-date"><?php echo strftime('%B %e, %Y', strtotime($review->rev_date)); ?></p>
		</div><?php 
		...
		if( ( ($this->mtconf['user_report_review'] == 1 && $this->my->id > 0) || $this->mtconf['user_report_review'] == 0) || ( $this->my->id == $this->link->user_id && empty($review->ownersreply_text) )) {
			echo '<div class="review-reply-and-report">';
			if( ($this->mtconf['user_report_review'] == 1 && $this->my->id > 0) || $this->mtconf['user_report_review'] == 0) { 
				?><div class="review-report"><a href="<?php echo JRoute::_('index.php?option='.$this->option.'&amp;task=reportreview&amp;rev_id='.$review->rev_id) ?>"><?php echo JText::_( 'Report review' ); ?></a></div><?php 
			} 

			if( $this->my->id == $this->link->user_id && empty($review->ownersreply_text) && $this->mtconf['owner_reply_review'] == 1 ) { 
				?><div class="review-reply"><a href="<?php echo JRoute::_('index.php?option='.$this->option.'&amp;task=replyreview&amp;rev_id='.$review->rev_id) ?>"><?php echo JText::_( 'Reply review' ) ?></a></div><?php 
			}
			echo '</div>';
		}
		?>
	...
	<p class="mbutton">
		<a href="<?php echo JRoute::_('index.php?option=com_mtree&task=writereview&link_id='.$this->link->link_id); ?>"><?php echo JText::_( 'WRITE REVIEW' ); ?><span class="lucida button-arrow">»</span></a>
	</p>
	...
	<p class="mbutton">
		<a href="<?php echo JRoute::_('index.php?option=com_mtree&task=writereview&link_id='.$this->link->link_id); ?>"><?php echo JText::_( 'Be the first to review' ); ?><span class="lucida button-arrow">»</span></a>
	</p>
	<?php endif; ?>
