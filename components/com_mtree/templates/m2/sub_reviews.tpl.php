<div class="reviews">
	...
	<div class="review"><div class="review-head"><div class="review-title"><?php 
		...
		if( ( ($this->mtconf['user_report_review'] == 1 && $this->my->id > 0) || $this->mtconf['user_report_review'] == 0) || ( $this->my->id == $this->link->user_id && empty($review->ownersreply_text) )) {
			...
			if( ($this->mtconf['user_report_review'] == 1 && $this->my->id > 0) || $this->mtconf['user_report_review'] == 0) { 
				?><div class="review-report"><a href="<?php echo JRoute::_('index.php?option=com_mtree&amp;task=reportreview&amp;rev_id='.$review->rev_id) ?>" style="font-weight:normal"><?php echo JText::_( 'Report review' ); ?></a></div><?php 
			} 

			if( $this->my->id == $this->link->user_id && empty($review->ownersreply_text) && $this->mtconf['owner_reply_review'] == 1 ) { 
				?><div class="review-reply"><a href="<?php echo JRoute::_('index.php?option=com_mtree&amp;task=replyreview&amp;rev_id='.$review->rev_id) ?>" style="font-weight:normal"><?php echo JText::_( 'Reply review' ) ?></a></div><?php 
			}
			echo '</div>';
		}
		?>
	...
	<p />
	<?php $this->plugin('ahref', "index.php?option=com_mtree&amp;task=writereview&amp;link_id=$this->link_id", JText::_( 'Be the first to review' )); ?>
