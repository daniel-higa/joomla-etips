jQuery(document).ready(function(){
	...
	if(jQuery("#sortableimages").length>0)
	{
		...
		jQuery('li input:checkbox').click(function(){
			enforceImageLimit();
...
function addAtt() {
	if((attCount + jQuery('li input:checkbox:checked').length)<=maxAtt) {
...
function enforceImageLimit() {
	var attTotal = attCount + jQuery('li input:checkbox:checked').length;
