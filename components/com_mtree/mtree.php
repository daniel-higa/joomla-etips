<?php
/**
 * @version		$Id: mtree.php 1042 2011-02-22 07:18:40Z cy $
 * @package		Mosets Tree
...
function search( $option ) {
	global $savantConf, $Itemid, $custom404, $mtconf;
	...
	if($restriction)
	{
		$app->enqueueMessage(JText::sprintf('SEARCH_MESSAGE',$mtconf->get('limit_min_chars'),$mtconf->get('limit_max_chars')));
	}
	...
	if(!empty($search_cat) && empty($searchword)) {
		$app->redirect( JRoute::_("index.php?option=$option&task=listcats&cat_id=$search_cat&Itemid=$Itemid") );
	}
...
function listalpha( $cat_id, $alpha, $limitstart, $option ) {
	...
	$document->setTitle(JText::sprintf( 'List alpha by listings and cats', strtoupper($alpha), $cat_name ));
...
function viewowner( $user_id, $limitstart, $option ) {
	...
	if( !empty($owner) ) {
		$document->setTitle(JText::sprintf( 'Listing by', $owner->username ));
...
function viewusersreview( $user_id, $limitstart, $option ) {
	...
	if( count($owner) == 1 && $mtconf->get('show_review') ) {
		$document->setTitle(JText::sprintf( 'Reviews by', $owner->username ));
...
function viewusersfav( $user_id, $limitstart, $option ) {
	...
	if( count($owner) == 1 && $mtconf->get('show_favourite')) {
		$document->setTitle(JText::sprintf( 'Favourites by', $owner->username ));
...
function viewgallery( $link_id, $option ) {
	...
	if($link === false)	{
		...
		$document->setTitle(JText::sprintf( 'Gallery2', $link->link_name ));
...
function viewimage( $img_id, $option ) {
	...
	if($link === false)	{
		...
		$document->setTitle(JText::sprintf( 'Image2', $link->link_name ));
...
function viewlink( $link_id, $my, $limitstart, $option ) {
	...
	if($link === false)	{
		...
		$document->addCustomTag(' <script src="'.$mtconf->getjconf('live_site').'/components/com_mtree/js/jquery.fancybox-1.3.4.pack.js" type="text/javascript"></script>');
...
function viewlink_cache( $link, $limitstart, $fields, $params, $my, $option ) {
	...
	if ( !isset($link->link_id) || $link->link_id <= 0 ) {
		...
		# Get image ids
		$database->setQuery("SELECT img_id AS id, mtree.filename, alt FROM #__mt_images mtree LEFT JOIN btda0_mtree_img_alts alts ON mtree.filename = alts.filename WHERE link_id = '" . $link_id . "' ORDER BY ordering ASC");
		//SELECT img_id AS id, filename FROM #__mt_images WHERE link_id = '" . $link_id . "' ORDER BY ordering ASC");
		$images = $database->loadObjectList();
...
function send_report( $link_id, $option ) {
	...
	if ( $mtconf->get('show_report') == 0 ) {
		...
		if( mosMailToAdmin( $subject, $text ) )
		{
			...
			$app->redirect( JRoute::_("index.php?option=$option&task=viewlink&link_id=$link_id&Itemid=$Itemid"), JText::_( 'Report have been sent' ) );
		}
	}
...
function send_claim( $link_id, $option ) {
	...
	if ( $mtconf->get('show_claim') == 0 || $my->id <= 0 ) {
		...
		$uri =& JURI::getInstance();
		$text = JText::sprintf( 'Claim email', $uri->toString(array( 'scheme', 'host', 'port' )) . JRoute::_("index.php?option=com_mtree&task=viewlink&link_id=$link_id&Itemid=$Itemid"), $link->link_name, $link_id, $message );
		...
		if( mosMailToAdmin( $subject, stripslashes ($text) ) )
		{
			...
			$app->redirect( JRoute::_("index.php?option=$option&task=viewlink&link_id=$link_id&Itemid=$Itemid"), JText::_( 'Claim have been sent' ) );
		}
	}
...
function confirmdelete( $link_id, $option ) {
	...
	if ($mtconf->get('user_allowdelete') && $my->id == $link->user_id && $my->id > 0 ) {
		...
		if ( $mtconf->get('notifyadmin_delete') == 1 ) {
			...
			$body = JText::sprintf( 'Admin notify delete msg', $link->link_name, $link->link_name, $link->link_id, $my->username, $my_email, $link->link_created );
		...
		$app->redirect( JRoute::_("index.php?option=$option&task=viewowner&user_id=".$my->id."&Itemid=$Itemid"), JText::_( 'Listing have been deleted' ) );
...
function addreview( $link_id, $option ) {
	...
	if ( count($user_rev) > 0 &&  $mtconf->get('user_review_once') == '1') {
		...
		if ( $mtconf->get('notifyadmin_newreview') == 1 ) {
			...
			if ( $rev_approved == 0 ) {
				$subject = JText::sprintf( 'New review email subject waiting approval', $link->link_name );
				$msg = JText::sprintf( 'Admin new review msg waiting approval', $link->link_name, $rev_title, $author_name, $author_username, $author_email, stripslashes(html_entity_decode($rev_text)));
			} else {
				$uri =& JURI::getInstance();
				$subject = JText::sprintf( 'New review email subject approved', $link->link_name);
				$msg = JText::sprintf( 'Admin new review msg approved', $link->link_name, $uri->toString(array( 'scheme', 'host', 'port' )) . JRoute::_("index.php?option=com_mtree&task=viewlink&link_id=$link_id&Itemid=$Itemid"), $rev_title, $author_name, $author_username, $author_email, stripslashes(html_entity_decode($rev_text)));
			}
		...
		if ( $mtconf->get('needapproval_addreview') == 1 ) {
			$app->redirect( JRoute::_("index.php?option=$option&task=viewlink&link_id=$link_id&Itemid=$Itemid"), JText::_( 'Review will be reviewed' ) );
		} else {
			$app->redirect( JRoute::_("index.php?option=$option&task=viewlink&link_id=$link_id&Itemid=$Itemid"), JText::_( 'Review have been successfully added' ) );
		}
...
function votereview( $rev_id, $rev_vote, $option ) {
	...
	if( $result ) {
		$return = JText::sprintf( 'People find this review helpful', (($rev_vote == 1)? $review->vote_helpful +1:$review->vote_helpful), ($review->vote_total +1) );
...
function send_reportreview( $rev_id, $option ) {
	...
	if( $mtconf->get('user_report_review') == -1 || ($mtconf->get('user_report_review') == 1 && $my->id == 0) ) {
		...
		if( count($link) == 1 && $link->link_id > 0 ) {
			...
			$uri =& JURI::getInstance();
			$text = JText::sprintf( 'Report review email', $uri->toString(array( 'scheme', 'host', 'port' )) . JRoute::_("index.php?option=com_mtree&task=viewlink&link_id=$link->link_id&Itemid=$Itemid"), $your_name, $message, $link->rev_title, $link->link_name, $link->rev_text, $link->rev_text );
			...
			if( mosMailToAdmin( $subject, $text ) )
			{
				...
				$app->redirect( JRoute::_("index.php?option=$option&task=viewlink&link_id=$link->link_id&Itemid=$Itemid"), JText::_( 'Report have been sent' ));
			}
...
function send_replyreview( $rev_id, $option ) {
	...
	if ( !$mtconf->get('owner_reply_review') ) {
		...
		if( count($link) == 1 && empty($link->ownersreply_text) && $link->link_id > 0 && $my->id > 0 && $link->link_owner_user_id == $my->id ) {
			...
			if ( $rr_approved == 0 ) {
				$subject = JText::sprintf( 'New review reply email subject waiting approval', $link->link_name);
				$msg = JText::sprintf( 'Admin new review reply msg waiting approval', $my->name, $message, $link->rev_title, $link->link_name, $link->rev_text );
			} else {
				$subject = JText::sprintf( 'New review reply email subject approved', $link->link_name);
				$msg = JText::sprintf( 'Admin new review reply msg approved', $my->name, $message, $link->rev_title, $author_name, $author_username, $author_email, $link->rev_text );
			}
			...
			if( mosMailToAdmin( $subject, $msg ) )
			{
				...
				if ( $mtconf->get('needapproval_replyreview') == 1 ) {
					$app->redirect( JRoute::_("index.php?option=$option&task=viewlink&link_id=$link->link_id&Itemid=$Itemid"), JText::_( 'Reply review will be reviewed' ));
				} else {
					$app->redirect( JRoute::_("index.php?option=$option&task=viewlink&link_id=$link->link_id&Itemid=$Itemid"), JText::_( 'Reply review have been successfully added' ));
				}
			}
...
function send_recommend( $link_id, $option ) {
	...
	if ( $mtconf->get('show_recommend') == 0 || ($mtconf->get('user_recommend') == '1' && $my->id < 1) || $mtconf->get('user_recommend') == -1 ) {
		...
		$uri =& JURI::getInstance();
		$msg = JText::sprintf( 'Recommend msg',
		...
		$subject = JText::sprintf( 'Recommend subject', $your_name);
		...
		if  (!validateInputs( $friend_email, $subject, $msg ) ) {
			...
			$app->redirect( JRoute::_("index.php?option=$option&task=viewlink&link_id=$link_id&Itemid=$Itemid"), JText::sprintf( 'Recommend email have been sent', $friend_name) );
		}
	}
}
...
function send_contact( $link_id, $option ) {
	...
	if ( 
		...
		$message = JText::sprintf( 'Contact message', $your_name, $your_email, $link->link_name, $uri->toString(array( 'scheme', 'host', 'port' )) . JRoute::_( "index.php?option=$option&task=viewlink&link_id=$link_id&Itemid=$Itemid", false ), JRequest::getVar( 'message', '', 'post') );

		if (!$your_email || (mt_is_email($your_email)==false) ){
			echo "<script>alert (\"".JText::_( 'You must enter valid email' )."\"); window.history.go(-1);</script>";
			exit(0);
		}

		$subject = JText::sprintf( 'Contact subject', $mtconf->getjconf('sitename'), $link->link_name);
		...
		if  (!validateInputs( $email, $subject, $message ) ) {
			...
			$app->redirect( JRoute::_("index.php?option=$option&task=viewlink&link_id=$link_id&Itemid=$Itemid"), JText::_( 'Contact email have been sent' ));
		}
	}
...
function editlisting( $link_id, $option ) {
	...
	{
		if( $cat ) {
			$document->setTitle(JText::sprintf( 'Add listing2', $cat->cat_name));
		} else {
			$document->setTitle(JText::_( 'Add listing' ));
		}
	} else {
		$document->setTitle(JText::sprintf( 'Edit listing2', $link->link_name));
	}
...
function savelisting( $option ) {
	...
	if( isset($other_cats) && $other_cats[0] != 0 && empty($other_cats[0]) ) {
		$other_cats = array();
	}
	...
	# Allowed
		...
		if( !empty($active_cfs) ) {
			...
			foreach($active_cfs AS $cf_id => $v) {
				...
				$mFieldTypeObject = new $class(array('id'=>$cf_id,'params'=>$params[$cf_id]->params,'linkId'=>$row->link_id));
		...
		if(
			...
			if(is_writable($mtconf->getjconf('absolute_path').$mtconf->get('relative_path_to_listing_small_image')) && is_writable($mtconf->getjconf('absolute_path').$mtconf->get('relative_path_to_listing_medium_image')) && is_writable($mtconf->getjconf('absolute_path').$mtconf->get('relative_path_to_listing_original_image'))) {
				...
				if( $files_exceed_limit ) {
					...
					$redirectMsg .= JText::sprintf( 'Image is not saved because it exceeded file size limit', $image_upload_limit );
				}
		...
		if ( $isNew ) {
			...
			if ( $mtconf->get('notifyuser_newlisting') == 1 && ( $my->id > 0 || 
				...
				if ( $row->link_approved == 0 ) {
					$subject = JText::sprintf( 'New listing email subject waiting approval', $row->link_name);
					$msg = JText::_( 'New listing email msg waiting approval' );
				} else {
					$subject = JText::sprintf( 'New listing email subject approved', $row->link_name);
					$msg = JText::sprintf( 'New listing email msg approved', $row->link_name, $uri->toString(array( 'scheme', 'host', 'port' )) . JRoute::_("index.php?option=com_mtree&task=viewlink&link_id=$row->link_id&Itemid=$Itemid"),$mtconf->getjconf('fromname'));
				}
			...
			if ( $mtconf->get('notifyadmin_newlisting') == 1 ) {
				...
				if ( $row->link_approved == 0 ) {
					$subject = JText::sprintf( 'New listing email subject waiting approval', $row->link_name);
					$msg = JText::sprintf( 'Admin new listing msg waiting approval', $row->link_name, $row->link_name, $row->link_id, $author->name, $author->username, $author->email);
				} else {
					$subject = JText::sprintf( 'New listing email subject approved', $row->link_name);
					$msg = JText::sprintf( 'Admin new listing msg approved', $row->link_name, $uri->toString(array( 'scheme', 'host', 'port' )) . JRoute::_("index.php?option=com_mtree&task=viewlink&link_id=$row->link_id&Itemid=$Itemid"), $row->link_name, $row->link_id, $author->name, $author->username, $author->email);
				}
		...
		elseif( $row->link_published == 1 ) {
			...
			if ( $mtconf->get('notifyuser_modifylisting') == 1 && $my->id > 0 ) {
				...
				if ( $row->link_approved < 0 ) {
					$subject = JText::sprintf( 'Modify listing email subject waiting approval', $row->link_name);
					$msg = JText::sprintf( 'Modify listing email msg waiting approval', $row->link_name, $uri->toString(array( 'scheme', 'host', 'port' )) . JRoute::_("index.php?option=com_mtree&task=viewlink&link_id=$old->link_id&Itemid=$Itemid") );
				} else {
					$subject = JText::sprintf( 'Modify listing email subject approved', $row->link_name);
					$msg = JText::sprintf( 'Modify listing email msg approved', $row->link_name, $uri->toString(array( 'scheme', 'host', 'port' )) . JRoute::_("index.php?option=com_mtree&task=viewlink&link_id=$old->link_id&Itemid=$Itemid"),$mtconf->getjconf('fromname'));
				}
			...
			if ( $mtconf->get('notifyadmin_modifylisting') == 1 ) {
				...
				if ( $row->link_approved < 0 ) {
					...
					$subject = JText::sprintf( 'Modify listing email subject waiting approval', $row->link_name);
					$msg .= nl2br(JText::sprintf( 'Admin modify listing msg waiting approval', $row->link_name, $uri->toString(array( 'scheme', 'host', 'port' )) . JRoute::_("index.php?option=com_mtree&task=viewlink&link_id=$old->link_id&Itemid=$Itemid"), $row->link_name, $row->link_id, $author->name, $author->username, $author->email, $diff_desc));

				} else {

					$subject = sprintf(JText::_( 'Modify listing email subject approved' ), $row->link_name);
					$msg .= nl2br(JText::sprintf( 'Admin modify listing msg approved', $row->link_name, $uri->toString(array( 'scheme', 'host', 'port' )) . JRoute::_("index.php?option=com_mtree&task=viewlink&link_id=$old->link_id&Itemid=$Itemid"), $row->link_name, $row->link_id, $author->name, $author->username, $author->email, $diff_desc));
...
function addcategory( $option ) {
	...
	$document->setTitle(JText::sprintf( 'Add cat2', $cat_name));
	...
	# Custom field's attachment
		...
		if( $my->id != $attachment->user_id && strpos(strtolower($my->usertype),'administrator') === false )
		{
			if( 
				...
				// Access denied.
				$app->redirect( JRoute::_('index.php?option=com_mtree&Itemid='.$Itemid), JText::_( 'You are not authorized to access this attachment' ) );
			}
		}
